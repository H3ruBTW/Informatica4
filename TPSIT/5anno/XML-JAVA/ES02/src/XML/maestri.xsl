   ...
   <xsl:template match = "/" >  
         ...
         <table border = "1" style="color:blue;font-size:40px" >
          ...
          <td > <xsl:for-each select="/maestri/*/artista">
           ...
           <tr> <td> <xsl:text> Query 18: Estrarre tutte le opere che seguono il primo artista nella lista dei pittori.</xsl:text> </td>     
           <td> <xsl:for-each select="/maestri/pittori/artista[1]/following::opera">
                <xsl:value-of select="concat('Opera: ', ., ', Anno: ', @anno)" />
                <br/>
                </xsl:for-each></td></tr>
            <tr>
            <td>
               <xsl:text> Query 19: estrae tutte le opere dei pittori con anno > 1500 e genere "olio su tela"</xsl:text>
            </td>
            <td>
               <xsl:for-each select="/maestri/pittori/artista/opera[@anno > 1500][@genere='statua']">
                  <xsl:value-of select="concat('Opera: ', ., ', Anno: ', @anno, ', Genere: ', @genere)"/>
                  <br/>
               </xsl:for-each>
            </td>
         </tr>
    ...
    <td> 
        <xsl:for-each select="/maestri/*/artista[count(opera) > 3]">
            <xsl:value-of select="@nome"/>
            <br/>
        </xsl:for-each>
    </td>
