...
function Login($usr, $psw, $cookies){
    ...
    try {
        ...
        if($ris){
            if(mysqli_num_rows($ris)>0){
                ...
                if($acc['Errori'] == 5){
                    ...
                    if((time()-$ultimo_errore)/60 < 30){
                        header("Location: Login.php?error=Troppi accessi riprova fra circa 30 minuti");
                        exit;
                    } else {
                        $user = $acc['Username'];
            ...
            //ERRORE QUERY
            header("Location: Login.php?error=Errore di ricerca dell'account. Riprovare più tardi.");
            exit;
        }
    } catch (\Throwable $th) {
